...
function displayResults(results) {
    ...
    resultsDiv.innerHTML = sortedResults.map((item, index) => {
        ...
        if (hasJapaneseChar) {
            // 日本語の場合：12文字×2行 = 24文字
            if (originalSynopsis.length > 23) {
                displaySynopsis = originalSynopsis.substring(0, 23) + '…';
            } else {
                displaySynopsis = originalSynopsis;
            }
            // 12文字で改行
            if (displaySynopsis.length > 12) {
                const line1 = displaySynopsis.substring(0, 12);
                const line2 = displaySynopsis.substring(12);
